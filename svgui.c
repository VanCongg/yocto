...
void *client_handler(void *arg)
{
    ...
    while (1)
    {
        ...
        // {
        //     char clients_list[BUFFER_SIZE] = {0};
        ...
        //         strcat(clients_list, client_info);
        //     }
        //     pthread_mutex_unlock(&lock);

        //     send(client_socket, clients_list, strlen(clients_list), 0);
        //     continue;
        // }
        ...
        if (sscanf(buffer, "SEND_FILE|%255[^|]|%255[^|]", filename, receiver) == 2)
        {
            ...
            if (!file_to_send)
            {
                ...
                send(client_socket, "ERROR|Failed to open file", 25, 0);
                continue;
            }
