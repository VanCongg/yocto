...
//     // Nhận dữ liệu nhiều lần nếu cần
//     while (1)
//     {
//         int bytes_received = recv(sockfd, buffer + total_received, BUFFER_SIZE - 1 - total_received, 0);
//         if (bytes_received <= 0)
//             break; // Ngừng nếu không nhận được nữa

//         total_received += bytes_received;
//         if (buffer[total_received - 1] == '\n')
//             break; // Dừng khi nhận đủ danh sách
//     }
//     buffer[total_received] = '\0';

//     printf("Danh sách client nhận được:\n%s\n", buffer);
...
void open_send_file_window(GtkWidget *widget, gpointer data)
{
    ...
    GtkWidget *combo_keysize = gtk_combo_box_text_new();
    ...
    // Hàng 5: Dropdown danh sách client online
    // combo_clients = gtk_combo_box_text_new();
    // gtk_box_pack_start(GTK_BOX(vbox), combo_clients, FALSE, FALSE, 5);
    ...
    // fetch_clients_from_server(sockfd, combo_clients);
    // Hàng 6: Nút gửi
